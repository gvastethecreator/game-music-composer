...
ROLE_AMP={'lead':.28,'counter':.19,'riff':.27,'bass':.27,'kick':.42,'snare':.32,'hat':.17,'tom':.28,'wood':.23,'ride':.14,'shaker':.13,'brush':.13,'rim':.18,'impact':.34,'comp':.17,'arp':.16,'motor':.18,'ostinato':.19,'pad':.12,'support':.16,'ensemble':.09,'pulse':.17,'texture':.08}
BUS_TARGET={'lead':-22.0,'bass':-23.0,'drums':-22.5,'rhythm':-25.0,'harmony':-26.0,'atmosphere':-28.5,'fx':-25.0}
BUS_EQ={'lead':(95,11000),'bass':(28,2600),'drums':(25,12500),'rhythm':(110,9500),'harmony':(145,8500),'atmosphere':(180,7500),'fx':(60,11000)}
BUS_COMP={'lead':(-21,2.0),'bass':(-22,2.6),'drums':(-18,3.0),'rhythm':(-23,2.0),'harmony':(-25,1.8),'atmosphere':(-27,1.6),'fx':(-20,2.5)}
BUS_SEND={'lead':.18,'bass':.035,'drums':.06,'rhythm':.11,'harmony':.16,'atmosphere':.24,'fx':.20}
CATEGORY_ADJUST={
 'action':{'drums':1.5,'bass':.8,'harmony':-1.0,'atmosphere':-1.5},'electronic':{'drums':1.2,'bass':1.0,'rhythm':.8,'atmosphere':-1},
 'urban':{'rhythm':.8,'bass':.5,'drums':.4},'classical':{'drums':-2.5,'harmony':.8,'atmosphere':1.2},
...
def render_style(style,outdir):
 ...
 if ogg.exists() and mp3.exists():
  ...
  return {'id':style['id'],'category':style['category'],'target_lufs':style.get('mix_v2',{}).get('target_lufs',-16),'integrated_lufs':round(lufs,3),'peak_dbfs':round(20*math.log10(peak+1e-12),3),'short_term_range_db':round(dyn,3),'quality_status':style.get('composition_quality',{}).get('status'),'events':len(style['events']),'skipped':True}
 ...
 for ev in style['events']:
  ...
  if ev['kind']=='drum':
   offset=int(max(0,ev.get('sample_offset_ms',0))*SR/1000);cents=round(float(ev.get('tuning_cents',0)),1);key=(sid,offset,cents,cutoff);y=DRUM_CACHE.get(key)
   if y is None:
    ...
    if len(DRUM_CACHE)<160:DRUM_CACHE[key]=y
   ...
   cents=round(float(ev.get('tuning_cents',0)),1);depth=round(float(ev.get('performance_vibrato_depth',ev.get('vibrato',0)) or 0),3);delay=round(float(ev.get('vibrato_delay_ms',140)),1);key=(sid,int(ev['midi']),round(dur,3),round(attack,3),round(release,3),cents,depth,delay,cutoff)
   ...
   if y is None:
    ...
    if cutoff:y=circular_filter(y[:,None].repeat(2,axis=1),None,cutoff)[:,0]
    if len(NOTE_CACHE)<260:NOTE_CACHE[key]=y
  amp=ROLE_AMP.get(role,.15)*float(ev.get('velocity_gain',1));st=pan_stereo(y*amp,float(ev.get('performance_pan',ev.get('pan',0))));put_circular(buses[bus],st,start)
 # process buses
 ...
 for b,x in buses.items():
  ...
  elif b in ('harmony','atmosphere'):x=width(x,1.18 if b=='harmony' else 1.28)
  elif b=='lead':x=width(x,.88)
  adjusted[b]=x.astype(np.float32)
 # priority automation
 leadg=sidechain_gain(adjusted['lead'],style.get('mix_v2',{}).get('lead_duck_db',1.8)) if np.any(adjusted['lead']) else np.ones(n,np.float32)
 ...
 if kick_starts and np.any(adjusted['bass']):
  env=make_trigger_env(n,kick_starts);duck=style.get('mix_v2',{}).get('kick_bass_duck_db',1.0);adjusted['bass']*=dbamp(-duck*env)[:,None]
 ...
 meter=pyln.Meter(SR);lufs=float(meter.integrated_loudness(mix));target=float(style.get('mix_v2',{}).get('target_lufs',-16));mix*=dbamp(target-lufs)
 # peak guard with conservative intersample margin
 peak=float(np.max(np.abs(mix))) or 1;ceiling=dbamp(float(style.get('mix_v2',{}).get('true_peak_dbfs',-1.0))-.25)

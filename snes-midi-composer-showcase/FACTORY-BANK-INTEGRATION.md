...
Neo-SPC Workstation v4.1 uses the Neo-SPC Factory Bank for the default **Live score** soundbank.

The workstation offers three live soundbanks over the same event data:

- **Factory Neo-16:** embedded multisample patches with ranges, velocity regions and round robins;
- **Original Mono:** the compact single-sample compatibility palette;
- **Chip Core:** pulse, triangle, noise and drum voices generated by Web Audio.

Changing a live bank keeps the cue, tempo, transpose and playback position. Mastered audio remains a separate playback source.

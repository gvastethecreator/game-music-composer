(() => {
  ...
  const els={};
  const state={view:'workstation',category:catalog.categories[0].id,style:null,pendingStyleId:null,selectionToken:0,search:'',quality:'all',mode:'live',tempo:100,transpose:0,volume:-3,ceiling:-1,seeking:false,playbackBusy:false,activeKeys:new Set(),ripples:[],particles:[],activity:new Map(),lastNow:performance.now(),raf:0,harnessSection:'brief',harnessConfig:null,bankProfile:'neo16',bankPatchId:factoryPatches[0]?.id||null,bankSearch:'',bankFamily:'all',bankVelocity:96,bankPreviewAudio:null,noticeTimer:0,reduceMotion:matchMedia('(prefers-reduced-motion: reduce)').matches};
  ...
  function savePrefs(){try{localStorage.setItem('neospc-studio-prefs',JSON.stringify({view:state.view,category:state.category,styleId:state.style?.id,mode:state.mode}))}catch{}}
  function syncUrl(){
    if(!state.style)return;try{const url=new URL(location.href);url.searchParams.set('cue',state.style.id);if(state.view==='workstation')url.searchParams.delete('view');else url.searchParams.set('view',state.view);history.replaceState(null,'',url)}catch{}
  }
  ...
  function renderTrackList(){const arr=filteredStyles();els.categoryCount.textContent=arr.length;els.trackList.innerHTML='';els.trackEmpty.hidden=arr.length>0;arr.forEach(s=>{const q=s.composition_quality?.status||'review',active=(state.pendingStyleId||state.style?.id)===s.id;const row=document.createElement('button');row.type='button';row.className='track-row'+(active?' active':'');row.setAttribute('aria-pressed',String(active));row.innerHTML=`<strong>${safe(s.title)}</strong><small>${safe(s.category_label)} · ${safe(s.subcategory)}</small><div class="row-meta"><span>${s.bpm} BPM</span><span>${s.meter}</span><span>${s.voice_budget} V</span><span class="quality-${q}">${q==='curated'?'approved':q}</span></div>`;row.addEventListener('click',()=>selectStyle(s.id,false));els.trackList.appendChild(row)})}

  function setupMasteredAudio(){els.audio.pause();els.audio.src=`assets/audio/${state.style.id}.mp3`;els.audio.playbackRate=state.tempo/100;els.audio.preservesPitch=true;els.audio.volume=clamp(Math.pow(10,state.volume/20),0,1);els.audioDownload.href=`assets/audio/${state.style.id}.mp3`;els.audioDownload.download=`${state.style.id}.mp3`}
  async function selectStyle(id,autoplay=false){
    ...
    try{
      const s=await loadCue(id);if(token!==state.selectionToken)return;state.style=s;state.pendingStyleId=null;state.activeKeys.clear();state.ripples=[];state.particles=[];state.activity.clear();engine.setStyle(s);engine.setTempo(state.tempo);engine.setTranspose(state.transpose);engine.setMasterDb(state.volume);engine.setCeilingDb(state.ceiling);setupMasteredAudio();
      ...
      els.quickPlay.textContent=`▶ Play ${s.title}`;els.quickPlay.setAttribute('aria-label',`Play ${s.title}`);els.quickHint.textContent=`${timeText(currentDuration())} loop · Factory Live · headphones recommended`;
  ...
  async function play(){
    if(state.playbackBusy||!state.style)return;setPlaybackBusy(true);els.transportState.textContent='LOADING';els.quickHint.textContent='Loading the Factory Bank…';
    try{
      if(state.mode==='live'){await ensureFactoryBank();engine.setStyle(state.style);engine.setTempo(state.tempo);engine.setTranspose(state.transpose);engine.setMasterDb(state.volume);engine.setCeilingDb(state.ceiling);await engine.play(currentBeat());els.transportState.textContent='PLAYING · LIVE'}
      else{els.audio.playbackRate=state.tempo/100;await els.audio.play();els.transportState.textContent='PLAYING · MASTER'}
      updateQuickPlayState();els.quickHint.textContent=`Now playing ${state.style.title} · ${state.mode==='live'?'Factory Live':'mastered preview'}`;
    }catch(err){els.transportState.textContent='AUDIO ERROR';els.quickHint.textContent='Playback could not start. Try Factory Live or allow audio in this browser.';notice('Playback could not start. Try Factory Live or allow audio in this browser.','error');console.error(err)}finally{setPlaybackBusy(false)}
  }
  ...
  function stop(){if(state.mode==='live'){engine.pause(false);engine.seekBeat(0)}else{els.audio.pause();els.audio.currentTime=0}els.scrubber.value=0;els.transportState.textContent='STOPPED';els.quickHint.textContent=`${timeText(currentDuration())} loop · Factory Live · headphones recommended`;updateQuickPlayState()}
  ...
  function bind(){
    ...
    els.mode.addEventListener('change',()=>switchMode(els.mode.value));els.volume.addEventListener('input',()=>setVolume(els.volume.value));els.ceiling.addEventListener('input',()=>setCeiling(els.ceiling.value));els.tempo.addEventListener('input',()=>setTempo(els.tempo.value));
    ...
    els.search.addEventListener('input',()=>{state.search=els.search.value;renderTrackList()});els.quality.addEventListener('change',()=>{state.quality=els.quality.value;renderTrackList()});els.clearTrackFilters.addEventListener('click',clearTrackFilters);
    ...
    els.audio.addEventListener('ended',()=>{els.transportState.textContent='READY';updateQuickPlayState()});els.audio.addEventListener('error',()=>{els.transportState.textContent='MASTER ERROR';if(state.mode==='mastered'){switchMode('live');notice('Mastered preview unavailable. Switched to Factory Live.','error')}});

...
SRC=pathlib.Path('/mnt/data/neospc_v4_work/data/neospc100-final-prepass.json')
QUALITY=pathlib.Path('/mnt/data/neospc_v4_work/reports/professor-review-final.json')
OUT=pathlib.Path('/mnt/data/neospc_v4_work/data/neospc100-mixpass.json')
...
def main():
 ...
 for s in d['styles']:
  ...
  q=quality.get(s['id'],{'score':70,'status':'revise','critique':[]})
  status={'approved':'curated','revise':'review','rebuild':'rebuild'}.get(q.get('status'),'review')
  s['composition_quality']={'score':q['score'],'status':status,'reasons':q.get('critique',[])}
  s['professor_review_final']=q
 d['version']='4.0.0-mixpass'; d['project']='Neo-SPC 100 / Full Rebuild Mix & Expression Pass';

...
The helper bounds target onset shifts by 25 ms and a fraction of the instrument's minimum written onset gap. It blends signed circular offsets so an early downbeat stays near the loop seam, and records loop-boundary crossings. It adjusts gate and velocity fields and checks exact symbolic overlaps under the legacy drum proxy. It refuses a resulting voice overflow without removing notes. It rejects applying itself twice; compare candidate amounts from the original, not a cumulatively degraded copy.
...
## Vector instrument direction

The shared SVG surfaces use a generated vector-style reference board: clean contours, three color planes per material and separate animated mechanisms. Note onset creates a velocity-scaled strike; sustained notes drive the bow, bellows and breath; release returns each part to rest. Focus shares the same attack age. Motion off and OS reduced motion keep pitch feedback without displacement. Strings, keys, bars and bells retain playable note targets. These are expressive illustrations, not physical fingering lessons.

...
function metrics(s){const ev=s.events;const offsets=ev.map(e=>((e.performedBeat??e.beat)-e.beat)*60000/s.bpm);const mean=offsets.reduce((a,b)=>a+b,0)/Math.max(1,ev.length);const std=Math.sqrt(offsets.reduce((a,b)=>a+(b-mean)**2,0)/Math.max(1,ev.length));let active=new Set();const grid=.125;let allSilent=0;for(let b=0;b<s.beats;b+=grid){if(!ev.some(e=>!resolve(e.patch).drum&&(e.beat<=b&&e.beat+e.duration>b)))allSilent++;}return{notes:ev.length,tracks:s.tracks.length,timingStd:std,meanOffset:mean,silence:allSilent/(s.beats/grid),seconds:s.beats*60/s.bpm};}

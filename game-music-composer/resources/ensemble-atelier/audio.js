...
function voiceSamples(patch,midi,duration,velocity,sr=32000){
 ...
 const a=1-Math.exp(-TAU*cutoff/sr);const count=Math.min(12,Math.floor((sr*.45)/f));
 for(let i=0;i<n;i++){
  ...
  if(p.drum){
   ...
   else{for(let k=1;k<=Math.min(count,7);k++){const partial=(k===1?1: .5/k)*Math.exp(-t*(.42+k*.45));x+=partial*Math.sin(phase*k*(1+.00007*k*k));}x+=noise*.045*Math.exp(-t/.008);if(sound==='prepared')x+=.18*Math.sin(phase*2.71)*Math.exp(-t*6);x*=.7;}
   ...
   for(let k=1;k<=Math.min(count,sound==='bass'?5:8);k++){const weight=(k===1?1:.55*Math.sin(k*.73)/k)*Math.exp(-t*(decay+k*.25));x+=Math.sin(phase*k)*weight;}
   x+=noise*.04*Math.exp(-t/.008);if(sound==='distorted')x=Math.tanh(x*2.2)*.42;else x*=.62;
   ...
   x=.64*Math.sin(phase);if(sound!=='ocarina')x+=.1*Math.sin(phase*2)+.07*Math.sin(phase*3);
   if(sound==='clarinet'||sound==='reed')x+=.21*Math.sin(phase*3)+.085*Math.sin(phase*5)+.04*Math.sin(phase*7);
   x+=noise*.012;
  }else if(sound==='choir'){
   ...
   for(let k=1;k<=Math.min(count,9);k++){x+=(Math.sin(phase*k)+.24*Math.sin(phase*k*1.0018))/(k*2.1);}
   x*=.52;x+=noise*.008;
   ...
   else{for(let k=1;k<=Math.min(count,8);k++){let amp=sound==='pulse'?(2*Math.sin(Math.PI*k*(p.id==='pulse25'?.25:.5))/(Math.PI*k)):1/(k*2.3);x+=amp*Math.sin(phase*k);}if(sound==='analogbass')x*=.85;}
...
class SoundEngine{
 constructor(){this.ctx=null;this.input=null;this.master=null;this.voices=new Set();this.cache=new Map();this.volume=.6;this.enabled=true;this.onVoice=null;this.previewCount=0;this.prepareCancel=null;this.maxCacheBytes=64*1024*1024;}
 ...
 note(event,when,duration,tr={volume:1,pan:0},ctx=this.ctx,dest=this.input,register=true){if(!ctx)return null;const source=ctx.createBufferSource(),gain=ctx.createGain(),pan=ctx.createStereoPanner();source.buffer=this.buffer(ctx,event,duration);gain.gain.value=M.clamp(tr.volume??1,0,1);pan.pan.value=M.clamp(tr.pan??0,-1,1);source.connect(gain);gain.connect(pan);pan.connect(dest);source.start(when);if(register){this.voices.add(source);source.onended=()=>{source.disconnect();gain.disconnect();pan.disconnect();this.voices.delete(source);};}return source;}
 panic(){if(!this.ctx)return;for(const source of this.voices){try{source.stop();}catch{}}this.voices.clear();}
 async preview(patch,midi,duration=.5,velocity=.7,tr={volume:.8,pan:0}){await this.init();const e={patch,midi,velocity};this.note(e,this.ctx.currentTime+.005,duration,tr);this.previewCount++;return{time:this.ctx.currentTime+.005,end:this.ctx.currentTime+.005+duration,event:e};}

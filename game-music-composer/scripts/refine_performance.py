...
def refine_style(style: dict[str, Any], profile: str = "chamber", amount: float = .5,
    ...
    for e in result["events"]:
        ...
        target_start = beat + target_shift*bpm/60
        mixed = (1-amount)*e.get("performance_beat", beat) + amount*target_start
        bounded = max(0, min(total-1e-8, mixed))
        boundary_adjustments += int(bounded != mixed)
        e["performance_beat"] = round(bounded, 8)
        e["start_offset_ms"] = round((bounded-beat)*60/bpm*1000, 5)
        if e["kind"] == "note":

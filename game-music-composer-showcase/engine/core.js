...
const session={
 ...
 ops:{commitHarmony:(...a)=>commitHarmony(...a),reworkHarmony:(...a)=>reworkHarmony(...a),setPerformanceMode:(...a)=>setPerformanceMode(...a),applyPerformanceTemplate:(...a)=>applyPerformanceTemplate(...a),bake:(...a)=>bake7(...a),transform:(...a)=>transform7(...a),ensureClip:(...a)=>ensureClip7(...a),duplicateClip:(...a)=>duplicateClip7(...a),deleteClip:(...a)=>deleteClip7(...a),changeBinding:(...a)=>changeBinding7(...a),collectUnusedClips:()=>collectUnusedClips7(),storeBank:(...a)=>storeBank7(...a),loadBank:(...a)=>{const game=state.gmc;loadBank7(...a);if(game&&!state.gmc){state.gmc=clone(game);persist();}},structureSong:()=>structureSong7(),
  // Song sections (UMBRA 7 behaviour without its dialogs).
  selectSection(id){const st=studio7(),sec=st.sections.find(s=>s.id===id);if(!sec)return false;if(player.playing)player.stop(false);saveSection7();for(const k of frameFields7)if(sec.frame[k]!==undefined)state[k]=clone(sec.frame[k]);st.bindings=clone(sec.frame.bindings||{});st.selectedSection=id;ui.bar=0;ui7.selection.clear();ui7.revision++;scoreMemo7.clear();host.render(state);persist();return true;},
  addSection(action='copy'){return commit7(()=>{ensureSections7();saveSection7();const st=studio7();if(st.sections.length>=12)throw Error('Esta vista admite hasta 12 secciones.');const current=st.sections.find(s=>s.id===st.selectedSection),labels={copy:current.name+' · copia',develop:'Desarrollo',contrast:'Tema B',reduce:'Ruptura',close:'Cierre',intro:'Intro'},name=labels[action]||'Sección';let f=action==='copy'?clone(current.frame):freezeFrame7(current.frame,name,action);
   if(action==='copy'){for(const [track,id]of Object.entries(f.bindings)){const c=st.clips[id],copy=createClip7(track,c.notes,{...c,name:c.name+' · copia'});f.bindings[track]=copy.id;}}
   const sec={id:id7('sec'),name,bars:f.bars,repeat:1,frame:f};st.sections.splice(st.sections.indexOf(current)+1,0,sec);},{message:'Sección independiente creada.'});},
  updateSection(id,{name,bars,repeat}={}){return commit7(()=>{saveSection7();const st=studio7(),sec=st.sections.find(x=>x.id===id);if(!sec)throw Error('Sección inválida.');if(name!==undefined){const n=String(name).trim().slice(0,70);if(n)sec.name=n;}if(repeat!==undefined)sec.repeat=clamp(Math.round(Number(repeat)),1,8);
   if(bars!==undefined&&Number(bars)!==sec.bars){bars=Number(bars);if(![1,2,3,4,6,8,12,16,24,32,48,64].includes(bars)||bars<sec.frame.degrees.length)throw Error('Duración de sección inconsistente.');sec.bars=sec.frame.bars=bars;for(const tr of sec.frame.tracks){const old=sec.frame.patterns[tr.id];sec.frame.patterns[tr.id]=Array.from({length:bars},(_,i)=>clone(old[i%old.length]));}if(st.selectedSection===sec.id){state.bars=bars;state.patterns=clone(sec.frame.patterns);ui.bar=Math.min(ui.bar,bars-1);}}},{restart:true});},
  removeSection(id){return commit7(()=>{saveSection7();const st=studio7();if(st.sections.length<2)return;st.sections=st.sections.filter(s=>s.id!==id);if(st.selectedSection===id){const next=st.sections[0];for(const k of frameFields7)if(next.frame[k]!==undefined)state[k]=clone(next.frame[k]);st.bindings=clone(next.frame.bindings);st.selectedSection=next.id;ui.bar=0;}},{restart:true});},
  automationTargets:()=>clone(targets7()),setSongMode:(...a)=>setSongMode7(...a),dropClip:(...a)=>dropClip7(...a),copyPhraseIntoTrack:(...a)=>copyPhraseIntoTrack7(...a),automate:(...a)=>automateControl7(...a),ensureLane:(...a)=>ensureLane7(...a),changeSound:(...a)=>changeSound8(...a),syncProjection:(...a)=>syncProjection7(...a),makeNote:(...a)=>makeNote7(...a),notesForTrack:id=>clone(notesForTrack7(id,state)),boundClip:id=>boundClip7(id,state),refreshHarmonyMaterial:()=>refreshHarmonyMaterial()}
};

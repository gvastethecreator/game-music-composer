(() => {
  ...
  const bankLabels={timber:'Timber',prism:'Prism',voltage:'Voltage',megadrive:'Mega Drive · sampled',snes:'SNES · sampled',velvet:'Velvet',circuit:'Circuit',factory:'Chamber',original:'Compact',chip:'Chip Core'};
  ...
  function setView(view,focus=false){
    const allowed=new Set(['workstation','recipe','soundbank','review']);state.view=allowed.has(view)?view:'workstation';document.body.dataset.view=state.view;
    ...
    $$('.app-view').forEach(panel=>{const active=panel.id===`view-${state.view}`;panel.classList.toggle('active',active);panel.hidden=!active});
  ...
  async function selectStyle(id,autoplay=false){
    ...
    try{
      const [loaded]=await Promise.all([loadCue(id),ensureFactoryBank()]);const s=studio.resolve(loaded);if(token!==state.selectionToken)return;if(state.style?.id!==s.id)state.bankMode=state.requestedBank||s.sound_palette||state.bankMode;state.requestedBank=null;state.style=s;state.pendingStyleId=null;state.activeKeys.clear();state.ripples=[];state.particles=[];state.activity.clear();els.soundbank.value=state.bankMode;renderSoundbankMode();engine.setStyle(s);engine.setBankMode(state.bankMode);engine.setTempo(state.tempo);engine.setTranspose(state.transpose);engine.setMasterDb(state.volume);engine.setCeilingDb(state.ceiling);setupMasteredAudio();
  ...
  async function play(){
    if(state.playbackBusy||!state.style)return;$$('.bank-console-packs audio').forEach(audio=>audio.pause());setPlaybackBusy(true);els.transportState.textContent='LOADING';
    try{

...
with sync_playwright() as p:
 ...
 page.locator('#play').click();page.wait_for_timeout(70)
 check('tempo is locked during preparation',page.locator('#tempo').is_disabled())
 page.locator('#stop').click();page.wait_for_timeout(300)
 ...
 for surface in ['keys','fret','bow','harp','wind','brass','choir','bellows','mallet','bells','tines','synth','drum']:
  ...
  if surface=='bells':check('bell mechanism follows the actual score pitch',page.evaluate("!!LAB.stage.dialog.querySelector('[data-motion=bell][data-tone=\"'+LAB.stage.score.events[0].midi+'\"]')"))
  # Visual snapshots of these surfaces are stored by the separate smoke run.
  page.keyboard.press('Escape')
 # A strike decays, focus shares its age, and a new strike restarts the motion.
 motion=page.evaluate("""()=>{const tr=MusicLab.track('solo','snare','drums'),s=MusicLab.makeScore('Strike',[tr]);MusicLab.add(s,'solo',0,.5,MusicLab.INST.snare.midi);MusicLab.finish(s);const stage=LAB.stage;stage.build(s,'solo');stage.focus('solo');stage.update(s.events,10);stage.update(s.events,10.04);const part=()=>stage.el.querySelector('[data-motion=stick]').style.transform,attack=part(),focused=stage.dialog.querySelector('[data-motion=stick]').style.transform;stage.update(s.events,10.24);const decay=part();stage.update([],11);stage.update([],11.3);const rest=part();stage.update(s.events,12);stage.update(s.events,12.04);return{attack,focused,decay,rest,retrigger:part()};}""")
 check('strike decay, focus continuity and retrigger',motion['attack']==motion['focused'] and motion['attack']!=motion['decay'] and motion['rest']=='' and motion['retrigger']==motion['attack'])
 check('motion off removes displacement and keeps pitch feedback',page.evaluate("()=>{LAB.stage.update(LAB.stage.score.events,13);document.querySelector('#view-motion').click();return LAB.stage.dialog.querySelector('svg').dataset.reduced==='true' && !!LAB.stage.dialog.querySelector('.lit') && [...LAB.stage.dialog.querySelectorAll('[data-motion]')].every(n=>!n.style.transform)}"))
 page.keyboard.press('Escape');page.locator('#view-motion').click()

...
def main():
    with sync_playwright() as p:
        ...
        page.goto(URL + '?cue=salsa_00_clave_patio&view=workstation')
        page.wait_for_function('document.querySelector("#studioOpenCreate") && window.GMCCreate')
        page.wait_for_timeout(1500)
        page.locator('#studioOpenCreate').click()
        check('Open in Create switches to Create', settle(page, 'document.body.dataset.view==="create"'))
        check('Catalog cue keeps its title in Create', 'Clave patio' in page.locator('#createTitle').text_content())
        page.locator('#createPlay').click()
        check('Catalog notes play through the synthesis engine', settle(page, 'GMCEngine.player.playing && GMCEngine.player.step>=4'))
        page.locator('#createPlay').click()

...
def build(output: Path) -> None:
    ...
    style='''<style>:root{color-scheme:dark}*{box-sizing:border-box}body{margin:0;background:#151713;color:#e5e8dc;font:15px/1.65 system-ui,sans-serif}main{max-width:1300px;margin:auto;padding:40px 28px}header,small{font:11px ui-monospace,monospace;color:#b3aa92;letter-spacing:.12em}header{padding:20px 28px;border-bottom:1px solid #41483d}h1{font-size:clamp(36px,5vw,60px);line-height:1.1;letter-spacing:-.05em}h2{font-size:25px;color:#debc85;margin-bottom:5px}h3{font-size:13px}p{color:#adb5a6;font-size:13px}.grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:18px;margin-top:30px}article{border:1px solid #444b3e;border-radius:12px;background:linear-gradient(#272b23,#1d221b);padding:24px;display:flex;flex-direction:column}article p{flex:1}.actions{display:flex;gap:8px;margin-top:16px}button{font:13px system-ui,sans-serif;border:1px solid #80745b;background:#393b2d;color:#ebdfc2;border-radius:5px;padding:11px 14px;cursor:pointer}button:focus-visible{outline:2px solid #f4ce8b;outline-offset:3px}.actions button:first-child{background:#c6a66f;color:#182012;flex:1}footer{margin-top:30px;border-top:1px solid #454a3e;padding-top:20px;font-size:12px;color:#9fa692}#viewer{display:none;position:fixed;inset:0;z-index:3;background:#141714;grid-template-rows:52px 1fr}#viewer.open{display:grid}.bar{display:flex;gap:15px;align-items:center;padding:7px 14px}#download-current{margin-left:auto}#frame{width:100%;height:100%;border:0}#viewer-label{font-size:12px}@media(max-width:950px){.grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:560px){main{padding:24px 16px}.grid{grid-template-columns:1fr}.bar{gap:7px}.bar button{padding:7px;font-size:11px}#viewer-label{max-width:140px;overflow:hidden;white-space:nowrap}}</style>'''
